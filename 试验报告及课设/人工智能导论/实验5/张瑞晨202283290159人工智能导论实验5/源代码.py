...
def main():
    ...
    parser.add_argument('--epochs', type=int, default=3, metavar='N',  # Change epochs to 3
    ...
    # Prepare to store training loss
    train_loss_values = []
    ...
    # Save the loss to a text file
    with open('train_loss_record.txt', 'w') as f:
        for epoch, loss in enumerate(train_loss_values, start=1):
            f.write(f'Epoch {epoch}: Test Loss: {loss:.4f}\n')

    # Save the loss curve as a plot
    plt.plot(range(1, args.epochs + 1), train_loss_values, label='Test Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss') 
